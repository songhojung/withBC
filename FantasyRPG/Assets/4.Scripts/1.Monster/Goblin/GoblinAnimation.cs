...
public class GoblinAnimation : MonoBehaviour {
    ...
    private G_STATE NowState = G_STATE.S_ATT1;
	// Use this for initialization
	void Start () {
        Goblin = this.gameObject.GetComponentInChildren<Animation>();
        ClipNum = Goblin.GetClipCount();
    ...
    private void Animation_Play3()
    {
        switch (NowState)
        {
            ...
            case G_STATE.S_ATT2:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_ATT2].name, 0.3f);
                break;
            case G_STATE.S_ATT3:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_ATT3].name, 0.3f);
                break;
            case G_STATE.S_BLOCK:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_BLOCK].name, 0.3f);
                break;
            case G_STATE.S_BLOCK_HIT:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_BLOCK_HIT].name, 0.3f);
                break;
            case G_STATE.S_COMBAT_IDLE:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_COMBAT_IDLE].name, 0.3f);
                break;
            case G_STATE.S_DEATH:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_DEATH].name, 0.3f);
                break;
            case G_STATE.S_REMOVE_WEAPON:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_REMOVE_WEAPON].name, 0.3f);
                break;
            case G_STATE.S_RUN:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_RUN].name, 0.3f);
                break;
            case G_STATE.S_WALK:
                Goblin.wrapMode = WrapMode.Loop;
                Goblin.CrossFade(PlayerAni[(int)G_STATE.S_WALK].name, 0.3f);
                break;
        }
