...
public class WerewolfeMove : MonoBehaviour {
    ...
    public WerewolfeAnimation WolfAni;
    ...
    void Move()
    {
        switch (WolfAni.NowState)
        {
            case WerewolfeAnimation.W_STATE.S_WALK:
                Vector3 VecSpider = (Vector3.forward * WalkSpeed * Time.deltaTime) + Wolf.transform.position;
                Wolf.MovePosition(VecSpider);
                break;
            case WerewolfeAnimation.W_STATE.S_RUN:
                Vector3 VecSpider2 = (Vector3.forward * WalkSpeed * 2.0f * Time.deltaTime) + Wolf.transform.position;
